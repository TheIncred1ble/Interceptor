...
namespace DllFilter {
    ...
    public:
        ...
        VOID Add(HMODULE hModule, ULONG Size, OPTIONAL PCUNICODE_STRING Name) {
            ...
            if (Name && Name->Buffer && Name->Length) {
                ...
                wmemcpy(std::data(Info.Name), Name->Buffer, Name->Length);
            }
            Lock.LockExclusive();
    ...
    DeclareHook(
        ...
        Log(std::wstring(L"Attempt to load library: ") + (!ModuleName.empty() ? ModuleName : L"UNKNOWN"));
            ...
            if (CheckResult != stValid) {
                ...
                case stWindowsHooks:
                    ...
                    if (Sfc::IsSystemFile(ModuleName.c_str())) {
                        Log(L"LdrLoadLibrary called from windows hooks handler, but allowed due to loading a system file: " + ModuleName);
                        FilterData.KnownModulesNames.Add(ModuleName);
                        return CallOriginal(LdrLoadDll)(PathToFile, Flags, ModuleFileName, ModuleHandle);
                    }
#endif // FEATURE_ALLOW_SYSTEM_MODULES
                    Log(L"LdrLoadLibrary called from windows hooks handler");
                ...
                case Notifier::tdAllow:
                    Log(L"LdrLoadLibrary allowed by external decision");
                    return CallOriginal(LdrLoadDll)(PathToFile, Flags, ModuleFileName, ModuleHandle);
                ...
                case Notifier::tdBlockOrTerminate:
                    ...
                    Log(L"LdrLoadLibrary denied (skipped) by external decision");
                    break;
                case Notifier::tdTerminate:
                    Log(L"LdrLoadLibrary caused fastfail by external decision");
                    __fastfail(0);
                    break;
                }
    ...
    BOOL EnableDllFilter(BOOL InitialCollectModulesInfo)
    {
        ...
        if (!FilterData.LdrRegisterDllNotification) {
            FilterData.LdrRegisterDllNotification = reinterpret_cast<_LdrRegisterDllNotification>(
                _GetProcAddress(Globals.hModules.hNtdll, "LdrRegisterDllNotification")
                );
        }
        ...
        if (!FilterData.LdrUnregisterDllNotification) {
            FilterData.LdrUnregisterDllNotification = reinterpret_cast<_LdrUnregisterDllNotification>(
                _GetProcAddress(Globals.hModules.hNtdll, "LdrUnregisterDllNotification")
                );
        }

        if (!FilterData.LdrRegisterDllNotification || !FilterData.LdrUnregisterDllNotification) {
            Log(L"Unable to initialize Ldr***DllNotification");
            return FALSE;
        }
        ...
        if (!NT_SUCCESS(Status) || !FilterData.Cookie) {
            Log(L"Unable to register Dll notification");
            return FALSE;
        }

        SetHookTarget(LdrLoadDll, _GetProcAddress(Globals.hModules.hNtdll, "LdrLoadDll"));
